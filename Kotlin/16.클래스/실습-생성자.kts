...
class User1 constructor(name: String) { // 클래스 네이밍은 대문자로 시작한다
    ...
    init { // 클래스가 생성될때(클래스를 통해서 객체를 만들때) 호출된다
        println(name)
        userName = name // 여기서 init이 초기화 블럭이므로 userName이 초기화 하지 않아도 됨
    }
}
...
// - constructor 키워드를 생략할수 없다 - 주생성자와 차이
...
class User6 constructor(name: String) {
    val name: String
    var age: Int = 0 // 부생성자에서 추가로 받는 인자들은 var로 선언하고 초기화해줘야함 (주생성자인 name제외)
    var nickname: String = ""
...
// 실행순서
// 부생성자 호출 -> 부생성자 안에있는 주생성자(this) 호출 -> init블록 호출 -> 부생성자 본문 실행 // init 다음 second
// 클래스 속성에서 초기화를 시켜주쥐 않아도 되는 이유
//  - 초기화블록(init블록)에서 초기화를 보장해주기 때문 -> 클래스가 생성될때 초기화블록은 무조건 실행된다
